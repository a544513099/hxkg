...
/**
	...
	public static void main1(String[] args) {
		...
		StringTokenizer st = new StringTokenizer(stvalue,
				"$");
		int count = 0;
...
//		System.out.println(getWinningNum("1", 10));
	...
	/**
	 * 抽奖
	 * @param winningRate    中奖概率
	 * @return
	 */
	public static int getWinningNum(String base, int winningRate) {
		int num = (int) Math.floor(base.length()*100/winningRate);
		int count = num-base.length();
		for (int i = 0; i < count; i++) {
			base += "0";
		}
	    Random random = new Random();     
	    int number = random.nextInt(base.length()); 
	    return Integer.parseInt(base.charAt(number)+"");
	 }     
